...
public class JogoGeneral extends JogoDados {
    ...
    // JogoDados, bem como seus próprios atributos:
    public JogoGeneral() {
    ...
    // O seguinte método retorna 'false' para jogadas já executadas e 'true' para as
    // disponiveis:
    ...
    // O seguinte metodo verifica qual jogada foi escolhida e retorna a pontuacao
    // que ela rende:
    public int pontuarJogada(int n) {
        ...
        for (i = 0; i < 5; i++) {
            armazenaValores[dadosJogoGeneral[i].getValue() - 1] += 1;
        }
        ...
        if (n == 1) {
            ...
            for (i = 0; i < 6; i++) {
                if (armazenaValores[i] >= 3) {
                    for (j = 0; j < 5; j++) {
                        soma += dadosJogoGeneral[j].getValue();
                    }
                    break;
                }
            }
            ...
            for (i = 0; i < 6; i++) {
                if (armazenaValores[i] >= 4) {
                    for (j = 0; j < 5; j++) {
                        sum += dadosJogoGeneral[j].getValue();
                    }
                    break;
                }
            }
