...
public class JogoAzar extends JogoDados {
    ...
    // Método construtor da classe JogoAzar, inicializando sua superclasse
    // JogoDados:
    public JogoAzar() {
    ...
    // Método getter que retorna o resultado (vitória-true ou derrota-false) do Jogo
    // de Azar:
    public boolean getResultado() {
        return this.resultado;
    }

    // Método que efetivamente faz o Jogo de Azar ser executado, retornando 'true'
    // no caso de uma vitória e 'false' no de uma derrota.
    public boolean executarRegrasJogo() {
        ...
        somarFacesSorteadas(dadosJogoAzar[0].getValue());
        somarFacesSorteadas(dadosJogoAzar[1].getValue());

        // Mostra-se para o usuário os valores dos dados:
        imprimirResultado();

        // Calcula-se e mostra-se para o usuário a soma das faces sorteadas:
        int somaFaces = dadosJogoAzar[0].getValue() + dadosJogoAzar[1].getValue();
        System.out.println("Valor total do lançamento: " + somaFaces);

        // Verifica-se a vitória ou a derrota do jogador. Se a soma das faces for 7 ou
        // 11, o jogador vence. Se for
        // 2, 3 ou 12, perde automaticamente. Caso contrário, ele pode rolar os dados
        // até 3 vezes mais, tentando
        ...
        if (somaFaces == 7 || somaFaces == 11) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                somarFacesSorteadas(dadosJogoAzar[0].getValue());
                somarFacesSorteadas(dadosJogoAzar[1].getValue());
                System.out.println("Valores dos dados do novo lançamento(" + (i + 2) + "): "
                        + dadosJogoAzar[0].getValue() + " e " + dadosJogoAzar[1].getValue());

                int total = dadosJogoAzar[0].getValue() + dadosJogoAzar[1].getValue();
    ...
    // O seguinte método sobrescreve 'imprimirResultado()', que é um método herdado
    // da superclasse JogoDados, para adequar-se
    ...
    public void imprimirResultado() {
        ...
        for (Dado dado : getDados()) {
            System.out.println("Valor do dado " + i + ": " + dado.getValue());
            i++;
        }
    }
}